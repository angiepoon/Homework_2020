...
ninstall.packages(c("ggplot2", "dplyr", "tidyr", "HistData", "mvtnorm",
                   "reticulate"))
```
...
```{r problem3b}
female <- subset(GaltonFamilies, gender == "female")
str(female) #453
male <- subset(GaltonFamilies, gender == "male")
str(male) #481
```
...
```{r problem3d}
First <- subset(GaltonFamilies, childNum == 1)
...
```{r problem3e}
male_mean <- mean(male$childHeight)
female_mean <- mean(female$childHeight)
mat <- matrix(c(male_mean, female_mean), ncol = 2)
colnames (mat) <- c("male", "female")
rownames(mat) <- "mean height"
tab <- as.table(mat)
tab
```
...
```{r subset}
sim_data_100 <- subset(sim_data, group < 101)
```
...
```{r cor, cache = TRUE}
dfcor_100 <- sim_data_100 %>%
  group_by(group) %>%
  summarise(cor = cor(X, Y, use = "everything", method = "pearson"))
head(sort(dfcor_100$cor, decreasing = T))

dfcor <- sim_data %>%
  group_by(group) %>%
  summarise(cor = cor(X, Y, use = "everything", method = "pearson"))

head(sort(dfcor$cor, decreasing = T))
```
...
subset(dfcor, cor > 0.8)
# group 99655
hist(subset(sim_data, group == 99655)$X)
```
...
names(myColors) <- levels(dat$Z)
colScale <- scale_colour_manual(name = "Z", values = myColors)

p <- ggplot(dat, aes(X, Y, color = Z)) + geom_point()
p1 <- p + colScale
```

Your text answer here.
The correlation of XY becomes positive
